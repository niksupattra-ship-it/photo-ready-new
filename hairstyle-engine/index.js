...
function cacheKey(portrait,mask,reference,hairId,provider){
 return crypto.createHash('sha256').update('v209|gpt-image-1.5|high|high|1024x1536|png|ai-hair-only-face-pixel-lock|')
...
export async function editHairstyle({portrait,mask,hairId,root=process.cwd(),env=process.env,fetcher=fetch}){
 ...
 form.append('prompt',`EDIT ONLY THE HAIRSTYLE IN IMAGE 1. Image 2 is the exact FACELESS TRANSPARENT HAIRSTYLE CUTOUT selected by the user, not a general inspiration image. ${styleInstruction} Reproduce Image 2's identifying geometry as closely as possible: exact part location, fringe or braid pattern, crown height, side volume, outline, length, direction and tapered ends. Adapt only its scale and volume to Image 1's existing skull, ears and unchanged face proportions. Create a photographic hairline with individual roots and fine strands that blend naturally into the existing forehead boundary; use HAIR PIXELS ONLY for this transition. Do not paint, patch, recolour or regenerate any forehead or face skin. The transparent pixels of the supplied mask are the only editable area; every opaque pixel must remain unchanged. Preserve the exact face, eyes, gaze, eyebrows, nose, mouth, jaw, ears, complexion, neck, shoulders, clothing and placement from Image 1. Never insert a face layer, skin rectangle, overlay band or hard geometric edge. Keep long hair outside the visible neck silhouette. This is a modest professional ID portrait. Return the same canvas size, framing and background with realistic roots, strand separation, subtle flyaways and no seams, halos or rectangular patches.`);
