...
function cacheKey(portrait,reference,hairId,provider){
 return crypto.createHash('sha256').update('v200|gpt-image-1.5|high|high|1024x1536|png|full-neck-shoulders-upper-chest|')
...
export async function editHairstyle({portrait,hairId,root=process.cwd(),env=process.env,fetcher=fetch}){
 ...
 form.set('prompt',`${form.get('prompt')} PIXEL-LEVEL FACE LOCK: Image 1 is the sole authority for the face. Preserve its exact face outline, forehead, cheeks, jaw, chin, natural asymmetry, eye shape and spacing, eyelids, eyebrow thickness and arch, nose, lips, ears, expression and age. Do not idealize, reshape, symmetrize, add makeup or transfer facial traits from Image 2. Preserve complexion, freckles, moles, marks, under-eye tone and fine lines. Improve only the clarity of existing natural pore detail; never smooth, whiten, airbrush, erase or invent skin features. If the original face is dark, apply only a restrained neutral exposure lift; otherwise preserve its exposure.`);
