...
SOURCE MICRO-TEXTURE PRESERVATION — V49: Image 1 is the sole authoritative source for facial skin detail. Preserve and resolve the EXISTING photographed pore pattern, fine skin grain, tiny lines, natural local contrast and real tonal variation from image 1 at high photographic fidelity. The goal is to recover visible source micro-detail that would be resolved by a sharply focused professional camera, NOT to invent or procedurally generate pores. Do not create a new skin surface, do not average skin tones, do not blur low-contrast texture, do not use beauty/portrait smoothing, and do not replace real texture with synthetic grain. Sharpen only genuine source-supported micro-detail with restrained optical clarity: no halos, ringing, crunchy edges or exaggerated pores. Facial geometry and every feature remain locked exactly as specified above.

HAIR — V9 HAIR-ONLY EDIT: ${keepOriginalHair?"KEEP THE ORIGINAL HAIR FROM IMAGE 1 EXACTLY. Do not restyle, replace, lengthen, shorten, recolor, thicken, thin, move the parting, change the fringe, change tied/untied structure, or alter the original hair silhouette. Preserve the subject's real hairline, roots, strands, flyaways, volume and visible hairstyle. The AI edit is required only so the natural neck region is generated while the original hair remains unchanged.":"CHANGE ONLY THE HAIR PIXELS needed to match image 2 as the authoritative hairstyle target. Never alter the face, skin, ears, eyebrows, eyes, nose, lips, jaw or facial geometry.  Match its parting, fringe, side shape, crown, volume, length, tied/untied structure and silhouette. Remove source-hair remnants that conflict with the selected style. Adapt the style to the subject's own skull, hairline, ears and head angle. Do not copy the reference face or anatomy. Hair must remain photographic, with natural roots, strands, density variation and soft flyaways."}
