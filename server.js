...
app.post("/api/ai-finish",upload.fields([{name:"image",maxCount:1},{name:"mask",maxCount:1}]),async(req,res)=>{
  try{
    ...
    const cleanPrompt=`CLEAN HEAD MASTER FOR A PROFESSIONAL ID PHOTO. This is a one-time anatomical reconstruction before hairstyle replacement. Remove ALL existing hair from the scalp, forehead, temples and behind the ears. Create a natural BALD scalp, complete anatomically plausible ears and an uncovered, continuous bare neck wherever hair used to obscure them. Extend that neck naturally downward for at least 55% of the face height below the chin, using the exact complexion, pores, tonal variation, lighting and shadows of Image 1. There must be no horizontal seam, pasted patch, flat painted skin, colour break or hard lower cut. Absolutely no remaining long strands, dark hair panels, sideburns, ponytail or hairline. Keep the person's face, expression, eyes, nose, mouth, jaw, original visible skin texture, complexion and head placement unchanged. Return a neutral professional head-and-long-neck crop on a plain temporary background; no clothing, torso or shoulders. This is an intermediate layer, not the final portrait.`;
...
GOAL: create one continuous, photorealistic head + hair + ears + LONG BARE NECK layer of the SAME PERSON for an ID portrait. Extend the natural neck downward for at least 55% of the visible face height below the chin so its lower end will remain hidden behind any wide or deep clothing-template collar. Preserve the subject as a real photographed person, not a beautified or re-rendered face. The application will place this layer behind its existing fixed clothing template, so DO NOT create or modify any clothing.
...
ABSOLUTE EXCLUSION MASK INSTRUCTION: every pixel belonging to forehead skin, temples, eyebrows, eyelashes, eyes, nose, cheeks, ears, lips, jaw, chin and the originally visible neck is protected and must remain governed exclusively by Image 1. Continue the missing lower neck from those same real skin pixels: exact complexion, pores, fine texture, tonal variation, side shadows and studio lighting. The transition from original to extended neck must be seamless and photographic, never a flat colour fill or cloned rectangular strip. Hair balancing, strand refinement and Pro Black 50% must affect HAIR PIXELS ONLY. Do not resize, warp, retouch, recolor or regenerate the face to make it fit the hairstyle; fit the hairstyle to the unchanged face instead.

OUTPUT / ANATOMY: centered front-facing ID-photo head, complete hair and ears, plus one uninterrupted natural bare neck extending at least 55% of face height below the chin. The neck must remain visible all the way to its lowest edge with consistent real skin texture and natural side shading. Do not generate shoulders or torso. No shirt, collar, tie, jacket, uniform, epaulettes, insignia, buttons or fabric. No horizontal neck cutoff, rectangular skin patch, colour band or visible compositing seam. Use a simple temporary solid background. Keep natural camera detail without halos or artificial sharpening.
