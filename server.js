...
app.post("/api/ai-finish",upload.fields([{name:"image",maxCount:1},{name:"mask",maxCount:1}]),async(req,res)=>{
  try{
    ...
    const cleanPrompt=`CLEAN HEAD MASTER FOR A PROFESSIONAL ID PHOTO. This is a one-time anatomical reconstruction before hairstyle replacement. Remove ALL existing hair from the scalp, forehead, temples and behind the ears. Create a natural BALD scalp, complete anatomically plausible ears, one continuous bare neck and the upper clavicle area wherever hair or clothing used to obscure them. Absolutely no remaining long strands, dark hair panels, sideburns, ponytail, hairline, shirt, jacket or rectangular image patch. Keep the person's face, expression, eyes, nose, mouth, jaw, original visible skin texture, complexion and head placement unchanged. Extend bare skin naturally to just below the clavicle line with balanced continuous lighting. Return a neutral professional head-neck-clavicle crop on a plain temporary background; no clothing or shoulders. This is an intermediate layer, not the final portrait.`;
...
GOAL: create one continuous, photorealistic head + hair + ears + bare neck + upper-clavicle skin layer of the SAME PERSON for an ID portrait. The bare skin must extend naturally to just below the clavicle line so any deep collar opening is fully filled. Preserve the subject as a real photographed person, not a beautified or re-rendered face. The application will place this anatomy layer behind its existing fixed clothing template, so DO NOT create or modify any clothing.
...
OUTPUT / ANATOMY: centered front-facing ID-photo head, complete hair and ears, one slightly long natural neck, and continuous bare upper-chest skin through just below both clavicles. Neck and clavicle colour, white balance, exposure, texture and side lighting must be continuous with the face, with no seam, doubled anatomy, pasted rectangle or hard horizontal edge. Do not generate clothed shoulders or torso. No shirt, collar, tie, jacket, uniform, black garment, epaulettes, insignia, buttons or fabric. Use a simple temporary solid background. Keep natural camera detail without halos or artificial sharpening.
