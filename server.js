...
app.post("/api/ai-finish",upload.fields([{name:"image",maxCount:1},{name:"mask",maxCount:1}]),async(req,res)=>{
  try{
    ...
    const cleanPrompt=`CLEAN HEAD MASTER FOR A PROFESSIONAL, FULLY CLOTHED ID PHOTO. This is a one-time hairstyle preparation step. Remove the existing hairstyle from the scalp, forehead, temples and behind the ears and reconstruct a natural scalp, complete ears and the person's complete natural neck region from the jawline down to the base of the neck. Keep normal human anatomy and proportions; the request is for complete crop coverage, not an anatomically elongated neck. Preserve the exact complexion, pores, tonal variation, studio lighting and natural side shadows from Image 1, with no seam, patch, flat colour or tonal break. Keep the face, expression, eyes, nose, mouth, jaw, original visible skin texture, complexion and head placement unchanged. The adult subject remains modestly and professionally dressed in a plain high-coverage crew-neck temporary shirt. Use a plain studio background. This is an intermediate professional ID-photo layer.`;
...
GOAL: create one continuous, photorealistic professional ID portrait of the SAME ADULT PERSON, including the complete head, hair, ears and natural neck region from the jawline down to the base of the neck. Keep normal human anatomy and proportions. Ensure the crop contains sufficient continuous neck coverage below the chin so a separate formal clothing template can overlap it cleanly. The subject must remain fully and modestly dressed in a plain high-coverage temporary shirt. Preserve the subject as a real photographed person, not a beautified or re-rendered face.
...
ABSOLUTE EXCLUSION MASK INSTRUCTION: every pixel belonging to forehead skin, temples, eyebrows, eyelashes, eyes, nose, cheeks, ears, lips, jaw, chin and the originally visible neck is protected and must remain governed exclusively by Image 1. Where the original crop does not contain the complete base of the neck, reconstruct only the minimal missing continuation using the same complexion, pores, fine texture, tonal variation, side shadows and studio lighting. The transition must be seamless and photographic, never a flat colour fill or cloned rectangular strip. Hair balancing, strand refinement and Pro Black 50% must affect HAIR PIXELS ONLY. Do not resize, warp, retouch, recolor or regenerate the face to make it fit the hairstyle; fit the hairstyle to the unchanged face instead.

OUTPUT / ANATOMY: centered front-facing, fully clothed professional ID portrait with complete hair, ears and a normal continuous neck visible from the jawline to its natural base. Preserve realistic neck width and length; do not stretch anatomy. Use a plain, high-coverage temporary crew-neck shirt without logos, insignia or accessories because the application will overlay its approved clothing template afterward. No horizontal cutoff, rectangular skin patch, colour band or visible compositing seam. Use a simple solid studio background. Keep natural camera detail without halos or artificial sharpening.
