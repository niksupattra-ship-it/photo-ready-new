...
app.post("/api/ai-finish",upload.fields([{name:"image",maxCount:1},{name:"mask",maxCount:1}]),async(req,res)=>{
  try{
    ...
    const cleanPrompt=`CLEAN HEAD MASTER FOR A PROFESSIONAL ID PHOTO. This is a one-time anatomical reconstruction before hairstyle replacement. Remove ALL existing hair from the scalp, forehead, temples and behind the ears. Create a natural BALD scalp, complete anatomically plausible ears, one continuous natural neck and visible clavicle area wherever hair or clothing used to obscure them. Absolutely no remaining long strands, dark hair panels, sideburns, ponytail, hairline, shirt, jacket or rectangular image patch. Keep the person's face, expression, eyes, nose, mouth, jaw, original visible skin texture, complexion and head placement unchanged. Build the neck in proportion to Image 1: the visible chin-to-clavicle length should normally be about 28–40% of the face height, adapted to this person's actual anatomy; the narrow middle-neck width should normally be about 65–80% of the jaw width; then widen smoothly and symmetrically toward both clavicles. These are guardrails, not fixed dimensions. Match the face's skin colour, texture, white balance, shadows and directional lighting continuously. Return a neutral professional head-neck-clavicle crop on a plain temporary background; no clothing. This is an intermediate layer, not the final portrait.`;
