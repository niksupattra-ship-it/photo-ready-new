...
FINAL PRIORITY: (1) same identity and face from Image 1, (2) restrained acne/dark-spot cleanup while retaining real pores and skin texture, (3) selected hairstyle only from Image 2 when supplied with the neck fully clear, (4) natural neck transition. Return a single coherent photographic person layer, not a face mask or pasted face.`
    ...
    // V210: lighting is a photographic requirement, never permission to redraw the locked face.
    const studioFlashGuidance=` STUDIO FILL-FLASH LIGHTING: balance facial exposure with the bright white uniform using a broad, soft, frontal camera-axis fill light. Gently open under-eye, nose-side, cheek and under-chin shadows without flattening all facial shape, blowing forehead/cheek highlights, changing skin colour or creating a shiny flash hotspot. Preserve fine ORIGINAL pore-scale contrast, freckles and natural photographic grain; do not hallucinate pores, airbrush, denoise or use a beauty filter. The protected face pixels remain unchanged by the AI; the application applies a restrained local skin-only shadow lift after generation. NECK FIT: use the original face/jaw width as the anatomical reference, not the temporary crop or the uniform's shoulder width. The middle neck should be approximately 70–85% of the jaw width where anatomically appropriate, with both sides widening smoothly under the jaw toward the collarbones; never a narrow stalk below an oversized head. Keep original head scale and facial proportions, avoid elongating the neck, and extend natural skin under both collar edges without clothing or lower torso.`;
    const requestPromptWithCoverage=`${requestPrompt} ${studioFlashGuidance} The generated person layer must contain an unbroken lower-neck fill region on both sides, widening naturally from the neck to approximately the midpoint between the neck and each shoulder joint, and continuing below the future collar edge. This covered compositing margin must match the face complexion and lighting and must not contain transparent gaps or background-coloured cut-outs.`;
