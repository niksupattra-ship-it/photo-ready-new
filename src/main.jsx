...
async function renderAdjustedFinal(headMasterBlob,lock,adjust,collarWarp=0){
 ...
 try{
  ...
  // Match the live CSS preview exactly: move, rotate and scale around the layer center.
...
function App(){
 ...
 const pick=e=>{const v=e.target.files?.[0];if(v){clearDiag();if(headMasterPreview)URL.revokeObjectURL(headMasterPreview);setHeadMasterPreview(null);setHeadPreviewLock(null);transparentCache.current={key:'',blob:null};editCache.current=null;const resetAdjust={scale:1,x:0,y:0,rotation:0};liveAdjustRef.current=resetAdjust;liveCollarWarpRef.current=0;setHeadAdjust(resetAdjust);setCollarWarp(0);setPreviewZoom(1);setPreviewPan({x:0,y:0});setComparePreview(false);setF(v);setA(URL.createObjectURL(v));setB();setMsg('')}};
 const applyAdjust=async next=>{liveAdjustRef.current=next;paintHeadTransform?.(next);setHeadAdjust(next);if(!editCache.current)return;try{const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,next,liveCollarWarpRef.current);showBlob(out)}catch(e){setMsg(e.message||'ปรับส่วนหัวไม่สำเร็จ')}};
 const nudge=(k,d)=>{const v={...headAdjust,[k]:headAdjust[k]+d};if(k==='scale')v.scale=Math.max(.20,Math.min(2.00,v.scale));applyAdjust(v)};
 const applyCollarWarp=async amount=>{const v=Math.max(-1,Math.min(1,amount));liveCollarWarpRef.current=v;setCollarWarp(v);if(!editCache.current)return;try{const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,liveAdjustRef.current,v);showBlob(out)}catch(e){setMsg(e.message||'ปรับช่องคอไม่สำเร็จ')}};
 ...
 const commitAdjust=async next=>{liveAdjustRef.current=next;setHeadAdjust(next);if(!editCache.current)return;try{const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,next,liveCollarWarpRef.current);showBlob(out)}catch(e){setMsg(e.message||'ปรับส่วนหัวไม่สำเร็จ')}};
 ...
 const downloadCurrentFinal=async()=>{
  ...
  try{
   ...
   const adjust={...liveAdjustRef.current};
   const warp=liveCollarWarpRef.current;
   const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,adjust,warp);
   showBlob(out);
 ...
 const previewPointerDown=e=>{
  ...
  else if(g.pointers.size>=2){g.pinch=false;g.drag=false}
 };
 const previewPointerMove=e=>{
  ...
  if(g.drag&&e.pointerId===g.primary){const next={...g.startAdjust,x:g.startAdjust.x+(e.clientX-g.x)/rect.width,y:g.startAdjust.y+(e.clientY-g.y)/rect.height};paintHeadTransform(next)}
 };
 ...
 const go=async()=>{setBusy(true);setMsg('');try{
  ...
  const masterPreviewURL=URL.createObjectURL(headNeckTransparent);setHeadMasterPreview(masterPreviewURL);setHeadPreviewLock(composed.lock);liveAdjustRef.current={scale:1,x:0,y:0,rotation:0};liveCollarWarpRef.current=0;
