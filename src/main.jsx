...
function App(){
 ...
 const gestureRef=useRef({drag:false,x:0,y:0,startX:0,startY:0,startAdjust:null,pinch:false,distance:0,zoom:1});
 ...
 const pick=e=>{const v=e.target.files?.[0];if(v){transparentCache.current={key:'',blob:null};editCache.current=null;setHeadAdjust({scale:1,x:0,y:0,rotation:0});setCollarWarp(0);setPreviewZoom(1);setComparePreview(false);setF(v);setA(URL.createObjectURL(v));setB();setMsg('')}};
 ...
 const previewPointerMove=e=>{const g=gestureRef.current;if(!g.drag||!b)return;e.preventDefault();const r=e.currentTarget.getBoundingClientRect(),next={...g.startAdjust,x:g.startAdjust.x+(e.clientX-g.x)/r.width/previewZoom,y:g.startAdjust.y+(e.clientY-g.y)/r.height/previewZoom};scheduleAdjust(next)};
 const previewPointerUp=e=>{if(gestureRef.current.drag){gestureRef.current.drag=false;clearTimeout(renderTimer.current);applyAdjust(headAdjust)}};
 const touchDistance=t=>Math.hypot(t[0].clientX-t[1].clientX,t[0].clientY-t[1].clientY);
 const previewTouchStart=e=>{if(!b)return;if(e.touches.length===2){e.preventDefault();gestureRef.current.pinch=true;gestureRef.current.distance=touchDistance(e.touches);gestureRef.current.zoom=previewZoom}else if(e.touches.length===1){gestureRef.current.drag=true;gestureRef.current.x=e.touches[0].clientX;gestureRef.current.y=e.touches[0].clientY;gestureRef.current.startAdjust={...headAdjust}}};
 const previewTouchMove=e=>{if(!b)return;const g=gestureRef.current;if(e.touches.length===2&&g.pinch){e.preventDefault();setPreviewZoom(Math.max(1,Math.min(4,g.zoom*touchDistance(e.touches)/Math.max(1,g.distance))))}else if(e.touches.length===1&&g.drag&&!g.pinch){e.preventDefault();const r=e.currentTarget.getBoundingClientRect(),next={...g.startAdjust,x:g.startAdjust.x+(e.touches[0].clientX-g.x)/r.width/previewZoom,y:g.startAdjust.y+(e.touches[0].clientY-g.y)/r.height/previewZoom};scheduleAdjust(next)}};
