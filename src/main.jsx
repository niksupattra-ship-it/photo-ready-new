...
function App(){
 ...
 const headPreviewLayerRef=useRef(null);
 const rafRef=useRef(0);
 const [masterPreviewUrl,setMasterPreviewUrl]=useState('');
 const masterPreviewUrlRef=useRef('');
 const setMasterPreviewBlob=blob=>{if(masterPreviewUrlRef.current)URL.revokeObjectURL(masterPreviewUrlRef.current);const u=URL.createObjectURL(blob);masterPreviewUrlRef.current=u;setMasterPreviewUrl(u)};
 const paintLiveHead=next=>{liveAdjustRef.current=next;const el=headPreviewLayerRef.current;if(!el||!editCache.current)return;const lock=editCache.current.lock;cancelAnimationFrame(rafRef.current);rafRef.current=requestAnimationFrame(()=>{const s=next.scale||1,r=next.rotation||0;el.style.left=`${(lock.hX/lock.W+(next.x||0))*100}%`;el.style.top=`${(lock.hY/lock.H+(next.y||0))*100}%`;el.style.transform=`translate3d(0,0,0) rotate(${r}deg) scale(${s})`;});};
 ...
 const pick=e=>{const v=e.target.files?.[0];if(v){clearDiag();transparentCache.current={key:'',blob:null};editCache.current=null;setHeadAdjust({scale:1,x:0,y:0,rotation:0});liveAdjustRef.current={scale:1,x:0,y:0,rotation:0};if(masterPreviewUrlRef.current){URL.revokeObjectURL(masterPreviewUrlRef.current);masterPreviewUrlRef.current='';setMasterPreviewUrl('')}setCollarWarp(0);setPreviewZoom(1);setPreviewPan({x:0,y:0});setComparePreview(false);setF(v);setA(URL.createObjectURL(v));setB();setMsg('')}};
 const applyAdjust=async next=>{liveAdjustRef.current=next;setHeadAdjust(next);paintLiveHead(next);if(!editCache.current)return;try{const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,next,collarWarp);showBlob(out)}catch(e){setMsg(e.message||'ปรับส่วนหัวไม่สำเร็จ')}};
 ...
 const scheduleAdjust=next=>{liveAdjustRef.current=next;setHeadAdjust(next);paintLiveHead(next);clearTimeout(renderTimer.current);renderTimer.current=setTimeout(()=>applyAdjust(next),180)};
 const previewPointerDown=e=>{if(!b||e.pointerType==='touch')return;e.preventDefault();e.currentTarget.setPointerCapture?.(e.pointerId);gestureRef.current={...gestureRef.current,drag:true,x:e.clientX,y:e.clientY,startAdjust:{...liveAdjustRef.current},startPan:{...previewPan},editHead:optionTool==='head'}};
 const previewPointerMove=e=>{if(e.pointerType==='touch')return;const g=gestureRef.current;if(!g.drag||!b)return;e.preventDefault();if(g.editHead){const r=e.currentTarget.getBoundingClientRect(),next={...g.startAdjust,x:g.startAdjust.x+(e.clientX-g.x)/r.width/previewZoom,y:g.startAdjust.y+(e.clientY-g.y)/r.height/previewZoom};paintLiveHead(next)}else if(previewZoom>1){setPreviewPan({x:g.startPan.x+(e.clientX-g.x),y:g.startPan.y+(e.clientY-g.y)})}};
 const previewPointerUp=e=>{if(e.pointerType==='touch')return;if(gestureRef.current.drag){const wasHead=gestureRef.current.editHead;gestureRef.current.drag=false;if(wasHead)applyAdjust({...liveAdjustRef.current})}};
 ...
 const previewTouchStart=e=>{if(!b)return;if(e.touches.length===2){e.preventDefault();const m=touchMid(e.touches);gestureRef.current.pinch=true;gestureRef.current.drag=false;gestureRef.current.distance=touchDistance(e.touches);gestureRef.current.zoom=previewZoom;gestureRef.current.midX=m.x;gestureRef.current.midY=m.y;gestureRef.current.startPan={...previewPan}}else if(e.touches.length===1){gestureRef.current.drag=true;gestureRef.current.x=e.touches[0].clientX;gestureRef.current.y=e.touches[0].clientY;gestureRef.current.startAdjust={...liveAdjustRef.current};gestureRef.current.startPan={...previewPan};gestureRef.current.editHead=optionTool==='head'}};
 ...
 const previewTouchEnd=e=>{if(e.touches.length<2)gestureRef.current.pinch=false;if(e.touches.length===0&&gestureRef.current.drag){const wasHead=gestureRef.current.editHead;gestureRef.current.drag=false;if(wasHead)applyAdjust({...liveAdjustRef.current})}};
 ...
 const go=async()=>{setBusy(true);setMsg('');try{
  ...
  const layer=aiLayer;
  editCache.current={master:headNeckTransparent,lock:composed.lock};setMasterPreviewBlob(headNeckTransparent);liveAdjustRef.current={scale:1,x:0,y:0,rotation:0};
  setHeadAdjust({scale:1,x:0,y:0,rotation:0});liveAdjustRef.current={scale:1,x:0,y:0,rotation:0};if(masterPreviewUrlRef.current){URL.revokeObjectURL(masterPreviewUrlRef.current);masterPreviewUrlRef.current='';setMasterPreviewUrl('')}setCollarWarp(0);
