...
function App(){
 ...
 const applyAdjust=async next=>{liveAdjustRef.current=next;setHeadAdjust(next);if(!editCache.current)return;try{const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,next,collarWarp);showBlob(out)}catch(e){setMsg(e.message||'ปรับส่วนหัวไม่สำเร็จ')}};
 ...
 const scheduleAdjust=next=>{liveAdjustRef.current=next;cancelAnimationFrame(adjustRafRef.current);adjustRafRef.current=requestAnimationFrame(()=>setHeadAdjust({...liveAdjustRef.current}));clearTimeout(renderTimer.current);renderTimer.current=setTimeout(()=>applyAdjust({...liveAdjustRef.current}),180)};
 const scheduleGestureAdjust=next=>{liveAdjustRef.current=next;cancelAnimationFrame(adjustRafRef.current);adjustRafRef.current=requestAnimationFrame(()=>setHeadAdjust({...liveAdjustRef.current}))};
 ...
 const previewTouchEnd=e=>{if(e.touches.length<2)gestureRef.current.pinch=false;if(e.touches.length===0&&gestureRef.current.drag){const wasHead=gestureRef.current.editHead;gestureRef.current.drag=false;clearTimeout(renderTimer.current);if(wasHead)applyAdjust({...liveAdjustRef.current})}};
 ...
 const go=async()=>{setBusy(true);setMsg('');try{
  ...
  setHeadAdjust({scale:1,x:0,y:0,rotation:0});liveAdjustRef.current={scale:1,x:0,y:0,rotation:0};setCollarWarp(0);
