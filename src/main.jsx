...
async function renderAdjustedFinal(headMasterBlob,lock,adjust,collarWarp=0,neckAdjust={width:0,length:0},backgroundPath='/assets/background.jpg',ribbonPath=null,ribbonAdjust={x:0,y:0,scale:1},collarPinPair=null){
 ...
 try{
  ...
  if(collarPinPair&&/\/government-uniforms\//.test(lock.templatePath||'')){
   ...
   if(isFemale){
    ...
    drawPin(leftPin,lock.uX+lock.uW*.315,y);
    drawPin(rightPin,lock.uX+lock.uW*.685,y);
   }else{
    const y=lock.uY+lock.uH*.245;
    drawPin(leftPin,lock.uX+lock.uW*.455,y);
    drawPin(rightPin,lock.uX+lock.uW*.545,y);
   }
  }
...
const COLLAR_PIN_OPTIONS=[
 {id:'garuda-pair',name:'เข็มตราครุฑคู่',left:'/assets/government-insignia/garuda-left.png',right:'/assets/government-insignia/garuda-right.png'}
];
...
function App(){
 ...
 const renderWithRibbon=(...args)=>renderAdjustedFinal(...args,ribbonRef.current,ribbonAdjustRef.current,collarPinRef.current);
...
{uniformCategory==='government'&&<button type="button" className={optionTool==='pins'?'active':''} onClick={()=>toggleOptionTool('pins')}><span className="line-tool-icon collar-pin-tool-icon" aria-hidden="true"><img src="/assets/government-insignia/garuda-left.png" alt=""/></span><strong>เข็ม</strong></button>}
