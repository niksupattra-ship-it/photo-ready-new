...
function App(){
 ...
 return <main className="app-shell"><header className="app-header"><div><div className="eyebrow">PHOTO READY</div><h1>รูปพร้อมใช้</h1><p>อัปโหลดรูป เลือกชุดและทรงผม แล้วจัดตำแหน่งให้พอดี</p></div><div className="step-badge">สร้างรูปติดบัตร</div></header><section className="workspace">
 <div className="control-column">
  <div className="ui-card"><div className="section-head"><span className="step-no">1</span><div><h2>เพิ่มรูป</h2><p>ใช้รูปหน้าตรง เห็นใบหน้าชัด</p></div></div><label className="upload"><input type="file" accept="image/*" onChange={pick}/>{a?<img src={a}/>:<><strong>แตะเพื่อเลือกรูป</strong><small>JPG · PNG · WEBP</small></>}</label></div>
  <div className="ui-card"><div className="section-head"><span className="step-no">2</span><div><h2>เลือกชุด</h2><p>เลือกประเภทก่อน แล้วเลือกรายละเอียดชุด</p></div></div>
   <div className="segmented">{[['job','สมัครงาน'],['government','ข้าราชการ'],['student','นักศึกษา'],['gown','ชุดครุย']].map(([id,n])=><button type="button" key={id} className={uniformCategory===id?'active':''} onClick={()=>setUniformCategory(id)}>{n}</button>)}</div>
   {uniformCategory==='job'&&<div className="choice-box"><div className="choice-title">ชุดสมัครงาน</div><div className="gender-tabs"><button className={gender==='male'?'active':''} onClick={()=>setGender('male')}>ชาย</button><button className={gender==='female'?'active':''} onClick={()=>setGender('female')}>หญิง</button></div><div className="option-chips">{(gender==='male'?['สูทชาย แบบ 1','สูทชาย แบบ 2','เชิ้ตขาว']:['สูทเรียบร้อย','สูทคอแบะ']).map(x=><span key={x} className="future-chip">{x}<small>เพิ่มไฟล์ภายหลัง</small></span>)}</div></div>}
   {uniformCategory==='government'&&<div className="choice-box"><label className="field-label">กระทรวง / สังกัด<select value={ministry} onChange={e=>setMinistry(e.target.value)}><option>กระทรวงการคลัง</option><option disabled>เพิ่มกระทรวงอื่นภายหลัง</option></select></label><div className="gender-tabs"><button className={gender==='male'?'active':''} onClick={()=>setGender('male')}>ชาย</button><button className={gender==='female'?'active':''} onClick={()=>setGender('female')}>หญิง</button></div><div className="level-grid">{[['operational','ปฏิบัติงาน'],['academic','ปฏิบัติการ'],['senior','ชำนาญการ / อาวุโส']].map(([id,n])=><button type="button" key={id} className={level===id?'active':''} onClick={()=>setLevel(id)}>{n}</button>)}</div><div className="current-template"><span className="status-dot"></span><div><strong>Template ชุดปัจจุบัน</strong><small>{ministry} · {gender==='male'?'ชาย':'หญิง'} · {level==='operational'?'ปฏิบัติงาน':level==='academic'?'ปฏิบัติการ':'ชำนาญการ / อาวุโส'}</small></div></div></div>}
   {uniformCategory==='student'&&<div className="choice-box"><div className="gender-tabs"><button className={gender==='male'?'active':''} onClick={()=>setGender('male')}>ชาย</button><button className={gender==='female'?'active':''} onClick={()=>setGender('female')}>หญิง</button></div><div className="empty-state">เตรียมโครงไว้แล้ว · เพิ่ม Template นักศึกษาภายหลัง</div></div>}
   {uniformCategory==='gown'&&<div className="choice-box"><label className="field-label">มหาวิทยาลัย<select disabled><option>เพิ่มมหาวิทยาลัยภายหลัง</option></select></label><div className="gender-tabs"><button className={gender==='male'?'active':''} onClick={()=>setGender('male')}>ชาย</button><button className={gender==='female'?'active':''} onClick={()=>setGender('female')}>หญิง</button></div><div className="empty-state">พร้อมเพิ่มชุดครุยแยกตามมหาวิทยาลัย</div></div>}
  </div>
  <div className="ui-card"><div className="section-head"><span className="step-no">3</span><div><h2>เลือกทรงผม</h2><p>เลือกจาก 29 แบบ</p></div></div><div className="hair-options">{HAIR_OPTIONS.map(h=><button type="button" key={h.id} className={'hair-card '+(hairId===h.id?'selected':'')} onClick={()=>setHairId(h.id)}><img src={h.src}/><span>{h.name.replace('ทรงผม ','')}</span></button>)}</div></div>
  <button className="primary-action" disabled={!f||busy} onClick={go}>{busy?'กำลังประมวลผล…':'สร้างรูป'}</button>{msg&&<div className="err">{msg}</div>}
 </div>
 <div className="preview-column"><div className="preview-sticky"><div className="preview-title"><div><h2>ตัวอย่าง</h2><p>{b?'ปรับตำแหน่งได้ทันที':'ผลลัพธ์จะแสดงที่นี่'}</p></div>{b&&<a className="save compact" href={b} download="photo-composed.png">ดาวน์โหลด</a>}</div><div className="preview-frame">{b?<img src={b}/>:a?<img src={a} className="source-preview"/>:<div className="preview-empty"><span>3:4</span><small>เพิ่มรูปเพื่อเริ่มต้น</small></div>}</div>
 {b&&<div className="post-tools"><div className="head-tools"><div className="tool-panel-header"><div><div className="head-tools-title">จัดตำแหน่งส่วนหัว</div><div className="tool-subtitle">ปรับขนาดและตำแหน่ง</div></div><button type="button" className="tool-reset" onClick={()=>applyAdjust({scale:1,x:0,y:0})}>คืนค่า</button></div><div className="slider-stack"><label className="slider-row"><div className="slider-meta"><span>ขนาด</span><strong>{Math.round(headAdjust.scale*100)}%</strong></div><input type="range" min="20" max="200" step="1" value={Math.round(headAdjust.scale*100)} onChange={e=>applyAdjust({...headAdjust,scale:Number(e.target.value)/100})}/></label><label className="slider-row"><div className="slider-meta"><span>ซ้าย — ขวา</span><strong>{headAdjust.x>=0?'+':''}{(headAdjust.x*100).toFixed(1)}%</strong></div><input type="range" min="-50" max="50" step="0.5" value={headAdjust.x*100} onChange={e=>applyAdjust({...headAdjust,x:Number(e.target.value)/100})}/></label><label className="slider-row"><div className="slider-meta"><span>บน — ล่าง</span><strong>{headAdjust.y>=0?'+':''}{(headAdjust.y*100).toFixed(1)}%</strong></div><input type="range" min="-50" max="50" step="0.5" value={headAdjust.y*100} onChange={e=>applyAdjust({...headAdjust,y:Number(e.target.value)/100})}/></label></div></div>
 <div className="collar-warp-panel"><div className="tool-panel-header"><div><div className="collar-warp-title">ช่องคอชุด</div><div className="tool-subtitle">บิดเฉพาะ Template ชุด</div></div><button type="button" className="tool-reset" onClick={()=>applyCollarWarp(0)}>คืนค่า</button></div><button type="button" className="collar-auto" onClick={autoFitCollar}>พอดีอัตโนมัติ</button><label className="slider-row collar-slider"><div className="slider-meta"><span>หุบ — ขยาย</span><strong>{Math.round(collarWarp*100)}%</strong></div><input type="range" min="-100" max="100" step="1" value={Math.round(collarWarp*100)} onChange={e=>applyCollarWarp(Number(e.target.value)/100)}/></label></div>
 <div className="ribbon-panel"><div><strong>แพรแถบ</strong><small>โครง Layer พร้อมเพิ่ม PNG ภายหลัง</small></div><span>เร็ว ๆ นี้</span></div></div>}
 </div></div></div>
 </section></main>
}
