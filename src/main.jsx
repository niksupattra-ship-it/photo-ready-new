...
async function renderAdjustedFinal(headMasterBlob,lock,adjust,collarWarp=0,neckAdjust={width:0,length:0},backgroundPath='/assets/background.jpg',ribbonPath=null,ribbonAdjust={x:0,y:0,scale:1},collarPinPair=null,collarPinAdjust={left:{x:0,y:0},right:{x:0,y:0}},preview=false,liveCanvas=null,collarHeight=0,chestPinPath=null,chestPinAdjust={x:0,y:0,scale:1}){
 ...
 const c=document.createElement('canvas');const ratio=preview?Math.min(1,Math.max(420,Math.round(liveCanvas?.clientWidth||420)*2)/lock.W):1;c.width=Math.round(lock.W*ratio);c.height=Math.round(lock.H*ratio);
  ...
  if(chestPinPath&&/\/government-uniforms\//.test(lock.templatePath||'')){
...
function App(){
 ...
 const renderWithRibbon=(...args)=>renderAdjustedFinal(...args,ribbonRef.current,ribbonAdjustRef.current,collarPinRef.current,collarPinAdjustRef.current,false,null,liveCollarHeightRef.current,chestPinRef.current,chestPinAdjustRef.current);
 ...
 const applyCollarHeight=amount=>{if(placementLocked)return;const v=Math.max(-1.6,Math.min(1.2,amount));liveCollarHeightRef.current=v;setCollarHeight(v);if(editCache.current){drawLivePreview();commitAdjust()}};
  ...
  try{
   ...
   if(!id&&initialProcessedMasterRef.current){
    ...
    if(cached){nextMaster=cached;setProgressStage(78,'กำลังใช้ภาพที่เคยสร้างไว้');}
     ...
     const aiHeadNeck=await aiFinishPortrait(firstUploadedPhoto,id||'');
 ...
 const chestPinControls=()=> <div className="head-adjust-modern head-adjust-with-steps ribbon-modern-adjust" role="tabpanel" aria-label="ปรับเข็มติดอก">{!chestPinId?<div className="ribbon-adjust-empty">เลือกเข็มติดอกก่อนปรับตำแหน่ง</div>:<>{[
  {key:'x',label:'ซ้าย–ขวา',type:'horizontal',min:-35,max:35,step:.5},
  {key:'y',label:'ขึ้น–ลง',type:'vertical',min:-35,max:35,step:.5},
  {key:'scale',label:'ขนาด',type:'scale',min:45,max:200,step:1}
 ].map(row=>{const isY=row.key==='y',isScale=row.key==='scale';const val=isScale?chestPinAdjust.scale*100:isY?-chestPinAdjust.y*100:chestPinAdjust.x*100;
 const change=number=>applyChestPinAdjust({...chestPinAdjust,[row.key]:(isY?-number:number)/100});
 return <div key={row.key} className="head-adjust-row"><span className="head-adjust-glyph" title={row.label}><HeadAdjustGlyph type={row.type}/></span><button type="button" className="head-slider-step" aria-label={`ลด${row.label}`} onClick={()=>change(Number(stepRangeValue(val,-row.step,row.min,row.max)))}>−</button><input aria-label={`เข็มติดอก ${row.label}`} type="range" min={row.min} max={row.max} step={row.step} value={val} onChange={e=>change(Number(e.target.value))}/><button type="button" className="head-slider-step" aria-label={`เพิ่ม${row.label}`} onClick={()=>change(Number(stepRangeValue(val,row.step,row.min,row.max)))}>+</button><output>{!isScale&&val>=0?'+':''}{Math.round(val)}{isScale?'%':'%'}</output><button type="button" className="head-row-reset" aria-label={`คืนค่า${row.label}`} onClick={()=>change(isScale?100:0)}><ResetGlyph/></button></div>})}</>}</div>;
 const pinSideControls=(side,label)=>{const value=collarPinAdjust[side];return <section className="pin-side-adjust"><strong>{label}</strong><div className="head-adjust-row"><span className="head-adjust-glyph" title="ซ้าย–ขวา"><HeadAdjustGlyph type="horizontal"/></span><button type="button" className="head-slider-step" aria-label="ลดเข็ม" onClick={()=>applyCollarPinAdjust({...collarPinAdjust,[side]:{...value,x:Number(stepRangeValue(value.x*100,-1*0.5,-20.0,20.0))/100}})}>−</button><input aria-label={`${label} เลื่อนซ้ายขวา`} type="range" min="-20" max="20" step=".5" value={value.x*100} onChange={e=>applyCollarPinAdjust({...collarPinAdjust,[side]:{...value,x:Number(e.target.value)/100}})}/><button type="button" className="head-slider-step" aria-label="เพิ่มเข็ม" onClick={()=>applyCollarPinAdjust({...collarPinAdjust,[side]:{...value,x:Number(stepRangeValue(value.x*100,1*0.5,-20.0,20.0))/100}})}>+</button><output>{value.x>=0?'+':''}{Math.round(value.x*100)}%</output><button type="button" className="head-row-reset" onClick={()=>applyCollarPinAdjust({...collarPinAdjust,[side]:{...value,x:0}})} aria-label={`${label} คืนค่าซ้ายขวา`}><ResetGlyph/></button></div><div className="head-adjust-row"><span className="head-adjust-glyph" title="ขึ้น–ลง"><HeadAdjustGlyph type="vertical"/></span><button type="button" className="head-slider-step" aria-label="ลดเข็ม" onClick={()=>applyCollarPinAdjust({...collarPinAdjust,[side]:{...value,y:-Number(stepRangeValue(-value.y*100,-1*0.5,-20.0,20.0))/100}})}>−</button><input aria-label={`${label} เลื่อนขึ้นลง`} type="range" min="-20" max="20" step=".5" value={-value.y*100} onChange={e=>applyCollarPinAdjust({...collarPinAdjust,[side]:{...value,y:-Number(e.target.value)/100}})}/><button type="button" className="head-slider-step" aria-label="เพิ่มเข็ม" onClick={()=>applyCollarPinAdjust({...collarPinAdjust,[side]:{...value,y:-Number(stepRangeValue(-value.y*100,1*0.5,-20.0,20.0))/100}})}>+</button><output>{-value.y>=0?'+':''}{Math.round(-value.y*100)}%</output><button type="button" className="head-row-reset" onClick={()=>applyCollarPinAdjust({...collarPinAdjust,[side]:{...value,y:0}})} aria-label={`${label} คืนค่าขึ้นลง`}><ResetGlyph/></button></div></section>};
 ...
 if(screen==='home'){
  ...
  return <main className="profile-home studio-home">
   <header className="studio-home-topbar"><div className="studio-home-brand"><span className="studio-logo" aria-hidden="true">▣</span><div><strong>PhotoID Studio TH</strong><small>สร้างรูปถ่าย สมัครงาน · ข้าราชการ · นักศึกษา · ชุดครุย</small></div></div><div className="studio-top-actions"><button type="button" onClick={()=>setHomeInfoOpen(v=>!v)} aria-expanded={homeInfoOpen}>ⓘ <span>วิธีใช้งาน</span></button><span className="studio-pro">♛ PRO</span></div></header>
   ...
   {homeFilter==='all'&&<section className="studio-featured"><div className="studio-featured-head"><div><h2>▣ &nbsp; ตัวอย่างยอดนิยม</h2><p>ตัวอย่างรูปที่ลูกค้าเลือกใช้มากที่สุด</p></div><div className="studio-featured-filter" role="group" aria-label="กรองตัวอย่าง">{[['all','ทั้งหมด'],['male','ผู้ชาย'],['female','ผู้หญิง']].map(([id,name])=><button type="button" key={id} className={homeShowGender===id?'active':''} onClick={()=>setHomeShowGender(id)}>{name}</button>)}</div></div><div className="studio-featured-list">{featured.filter(c=>homeShowGender==='all'||c.gender===homeShowGender).map((c,i)=><button type="button" key={c.id||i} className="studio-featured-item" onClick={()=>openFeatured(c)}><img src={c.img} alt={c.title} loading="lazy"/><strong>{c.title}</strong></button>)}</div></section>}
...
{uniformCategory==='government'&&<button type="button" className={optionTool==='chest-pins'?'active':''} onClick={()=>toggleOptionTool('chest-pins',()=>setChestPinPanelTab('select'))}><span className="line-tool-icon collar-pin-tool-icon" aria-hidden="true"><img src="/app-icons/tool-collar-pins.png" alt=""/></span><strong>เข็มติดอก</strong></button>}
