...
function App(){
 ...
 return <main className="app-shell modern-shell"><header className="mobile-topbar"><button type="button" className="detail-back" onClick={()=>setScreen('home')}>‹</button><div><div className="eyebrow">PHOTO READY</div><h1>เลือกสไตล์</h1></div><div className="step-badge">ของฉัน</div></header><section className="modern-flow">
  ...
  <section className="gallery-section"><div className="gallery-heading"><div><h2>{uniformCategory==='job'?'ชุดสมัครงาน':uniformCategory==='government'?'ชุดข้าราชการ':uniformCategory==='student'?'ชุดนักศึกษา':'ชุดครุย'}</h2><p>เลือกแบบที่ต้องการ</p></div><span>เพิ่มเติม ›</span></div><div className="style-strip">
   ...
   {uniformCategory==='job'&&(gender==='male'?['สูทชาย 1','สูทชาย 2','เชิ้ตขาว']:['สูทเรียบร้อย','สูทคอแบะ']).map((x,i)=><button type="button" className="style-card future" key={x}><div className="style-placeholder">{i+1}</div><strong>{x}</strong><small>เพิ่ม Template ภายหลัง</small></button>)}
   {uniformCategory==='student'&&['นักศึกษา ชาย','นักศึกษา หญิง'].map((x,i)=><button type="button" className="style-card future" key={x}><div className="style-placeholder">{i+1}</div><strong>{x}</strong><small>เพิ่ม Template ภายหลัง</small></button>)}
   {uniformCategory==='gown'&&<button type="button" className="style-card future"><div className="style-placeholder">+</div><strong>เลือกมหาวิทยาลัย</strong><small>เพิ่มภายหลัง</small></button>}
  </div></section>
  <section className="style-detail-card"><div className="detail-title"><h2>เลือกสไตล์</h2><span>แตะตัวเลือกด้านล่าง</span></div><label className="hero-preview preview-upload"><input type="file" accept="image/*" onChange={pick}/>{b?<img src={b}/>:a?<><img src={a} className="source-preview"/><span className="change-photo">เปลี่ยนรูป</span></>:<div className="preview-empty"><span className="add-photo">+ เพิ่มรูป</span><small>JPG · PNG · WEBP</small></div>}</label>
   ...
   <div className="hair-picker-head"><div><h3>ทรงผม</h3><p>เลือกจาก 29 แบบ</p></div><span>{hairId.replace('hair-','')}</span></div><div className="hair-carousel">{HAIR_OPTIONS.map(h=><button type="button" key={h.id} className={'hair-card '+(hairId===h.id?'selected':'')} onClick={()=>setHairId(h.id)}><img src={h.src}/><span>{h.name.replace('ทรงผม ','')}</span></button>)}</div>
   <button className="primary-action create-now" disabled={!f||busy} onClick={go}>{busy?'กำลังประมวลผล…':'สร้างตอนนี้'}</button>{msg&&<div className="err">{msg}</div>}
  </section>
