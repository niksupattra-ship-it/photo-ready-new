...
async function renderAdjustedFinal(headMasterBlob,lock,adjust,collarWarp=0,neckAdjust={width:0,length:0},backgroundPath='/assets/background.jpg'){
...
const INTERIOR_UNIFORMS=[
 {id:'interior-01',name:'แบบ 1',img:'/assets/government-uniforms/interior-01.png'},
 {id:'interior-02',name:'แบบ 2',img:'/assets/government-uniforms/interior-02.png'},
 {id:'interior-03',name:'แบบ 3',img:'/assets/government-uniforms/interior-03.png'},
];
const GOVERNMENT_FINANCE_TEMPLATE='/assets/uniform.png';
...
function App(){
 ...
 const selectedGovernmentTemplate=ministry==='กระทรวงมหาดไทย'&&gender==='male'?selectedInteriorTemplate:GOVERNMENT_FINANCE_TEMPLATE;
 const selectGovernmentGender=(next)=>{setGender(next);if(next==='female'&&ministry==='กระทรวงมหาดไทย'){setMinistry('กระทรวงการคลัง');setAffiliationPreview(false);}};
 ...
 const pick=e=>{const v=e.target.files?.[0];if(v){backgroundRef.current='/assets/background.jpg';setBackgroundId('default');if(headMasterPreview)URL.revokeObjectURL(headMasterPreview);setHeadMasterPreview(null);setHeadPreviewLock(null);transparentCache.current={key:'',blob:null};editCache.current=null;setHeadAdjust({scale:1,x:0,y:0,rotation:0});liveAdjustRef.current={scale:1,x:0,y:0,rotation:0};setPlacementLocked(false);lockedPlacementRef.current=null;lockedMasterRef.current=null;hairResultCacheRef.current.clear();preparedHairBaseRef.current=null;lastHairDonorRef.current=null;setCollarWarp(0);liveCollarWarpRef.current=0;setNeckAdjust({width:0,length:0});liveNeckAdjustRef.current={width:0,length:0};setPlacementLocked(false);lockedPlacementRef.current=null;setPreviewZoom(1);setPreviewPan({x:0,y:0});setComparePreview(false);setF(v);setA(URL.createObjectURL(v));setB();setMsg('')}};
 const applyAdjust=async next=>{if(placementLocked)return;liveAdjustRef.current=next;paintHeadTransform?.(next);setHeadAdjust(next);if(!editCache.current)return;try{const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,next,liveCollarWarpRef.current,liveNeckAdjustRef.current,backgroundRef.current);showBlob(out)}catch(e){setMsg(e.message||'ปรับส่วนหัวไม่สำเร็จ')}};
 ...
 const applyNeckAdjust=async next=>{if(placementLocked)return;const v={width:Math.max(-1,Math.min(1,next.width||0)),length:Math.max(-1,Math.min(1,next.length||0))};liveNeckAdjustRef.current=v;setNeckAdjust(v);if(!editCache.current)return;try{const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,{...liveAdjustRef.current},liveCollarWarpRef.current,v,backgroundRef.current);showBlob(out)}catch(e){setMsg(e.message||'ปรับคอไม่สำเร็จ')}};
 ...
 const commitAdjust=async next=>{
  ...
  try{
   const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,next,liveCollarWarpRef.current,liveNeckAdjustRef.current,backgroundRef.current);
 ...
 const changeHair=async id=>{
  ...
  try{
   ...
   const out=await renderAdjustedFinal(nextMaster,editCache.current.lock,snap.adjust,snap.collarWarp,snap.neckAdjust,backgroundRef.current);
 ...
 const selectBackground=async option=>{
  ...
  try{
   const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,{...liveAdjustRef.current},liveCollarWarpRef.current,liveNeckAdjustRef.current,option.src);
   if(seq===renderSeqRef.current)showBlob(out);
 ...
 const downloadCurrentFinal=async()=>{
  ...
  try{
   clearTimeout(renderTimer.current);
   ++renderSeqRef.current;
   const out=await renderAdjustedFinal(editCache.current.master,editCache.current.lock,{...liveAdjustRef.current},liveCollarWarpRef.current,liveNeckAdjustRef.current,backgroundRef.current);
   showBlob(out);
 ...
 const go=async()=>{if(busy||hairBusy)return;++renderSeqRef.current;clearTimeout(renderTimer.current);setBusy(true);setMsg('');try{
  ...
  const finished=await renderAdjustedFinal(headNeckTransparent,composed.lock,{scale:1,x:0,y:0},0,{width:0,length:0},backgroundRef.current);
  showBlob(finished);
 ...
 if(screen==='home'){
  ...
  const governmentAffiliations=[{id:'finance',name:'กระทรวงการคลัง',img:GOVERNMENT_FINANCE_TEMPLATE},...(gender==='male'?[{id:'interior',name:'กระทรวงมหาดไทย',img:INTERIOR_UNIFORMS[0].img}]:[])];
  return <main className="profile-home">
   ...
   <section className="home-content">
    {homeFilter==='government'&&<section className="government-filter-panel"><div className="government-filter-title"><span>ข้าราชการ</span><h2>เลือกแบบชุด</h2></div><div className="government-gender-tabs"><button type="button" className={gender==='male'?'active':''} onClick={()=>setGender('male')}>ชาย</button><button type="button" className={gender==='female'?'active':''} onClick={()=>selectGovernmentGender('female')}>หญิง</button></div><div className="government-level-grid">{governmentLevels.map(([id,n])=><button type="button" key={id} className={level===id?'selected':''} onClick={()=>{setLevel(id);setSelectedStyle(n)}}><img src={selectedGovernmentTemplate}/><strong>{n}</strong><span className="selected-mark">✓</span></button>)}</div><div className="affiliation-section"><div className="affiliation-heading"><h3>เลือกสังกัด</h3><span>แสดงตามระดับที่เลือก</span></div><div className="affiliation-list">{governmentAffiliations.map(a=><button type="button" key={a.id} className="affiliation-card" onClick={()=>{setMinistry(a.name);setUniformCategory('government');setSelectedStyle((governmentLevels.find(x=>x[0]===level)?.[1]||'ชุดราชการ')+' · '+a.name);setAffiliationPreview(true)}}><img src={a.img}/><div><strong>{a.name}</strong><small>{gender==='male'?'ชาย':'หญิง'} · {governmentLevels.find(x=>x[0]===level)?.[1]}</small></div><b>›</b></button>)}</div>{affiliationPreview&&<div className="affiliation-preview"><div className="affiliation-preview-head"><div><span>ตัวอย่างชุดที่เลือก</span><strong>{selectedStyle}</strong></div></div><div className="affiliation-preview-card"><img src={selectedGovernmentTemplate} alt="ตัวอย่างชุดราชการ"/>{ministry==='กระทรวงมหาดไทย'&&gender==='male'&&<div className="interior-template-picker">{INTERIOR_UNIFORMS.map(t=><button type="button" key={t.id} className={selectedInteriorTemplate===t.img?'selected':''} onClick={()=>setSelectedInteriorTemplate(t.img)} aria-label={t.name} aria-pressed={selectedInteriorTemplate===t.img}><img src={t.img} alt={t.name}/><span>{t.name}</span></button>)}</div>}<button type="button" onClick={()=>setScreen('process')}>เลือกแบบนี้</button></div></div>}</div></section>}
 ...
 return <main className="app-shell modern-shell adaptive-editor"><header className="mobile-topbar process-mobile-topbar editor-context-header"><button type="button" className="detail-back" onClick={()=>{setScreen('home');setHomeFilter(uniformCategory==='government'?'government':uniformCategory)}} aria-label="กลับหน้าก่อนหน้า">‹</button><div><div className="eyebrow">PHOTO READY</div><h1>{selectedStyle||'สร้างรูป'}</h1></div><div className="step-badge">ของฉัน</div></header><section className="modern-flow">
  <section className="style-detail-card"><div className="detail-title process-page-title editor-preview-heading"><h2>เพิ่มรูป</h2><span>{selectedStyle||'แบบที่เลือก'}</span></div><label ref={previewStageRef} className={"hero-preview preview-upload "+(b?"direct-edit-preview":"")} onPointerDown={previewPointerDown} onPointerMove={previewPointerMove} onPointerUp={previewPointerUp} onPointerCancel={previewPointerUp} onWheel={previewWheel} onClick={e=>{if(a||b){e.preventDefault();if(optionTool&&optionTool!=='head')setOptionTool(null)}}}><input type="file" accept="image/*" onChange={pick}/>{b?<><img src={comparePreview&&a?a:b} className="editable-result-image final-render-preview"/><div className="preview-floating-actions"><button type="button" onClick={e=>{e.preventDefault();e.stopPropagation();setComparePreview(false);setPreviewZoom(1);setPreviewPan({x:0,y:0});applyAdjust({scale:1,x:0,y:0,rotation:0});applyCollarWarp(0)}} onPointerDown={e=>e.stopPropagation()} aria-label="รีเซ็ต"><span>↻</span><small>รีเซ็ต</small></button><button type="button" className={comparePreview?'active':''} onPointerDown={e=>e.stopPropagation()} onClick={e=>{e.preventDefault();e.stopPropagation();setComparePreview(v=>!v)}} aria-label="เปรียบเทียบ"><span>◐</span><small>เปรียบเทียบ</small></button></div>{!comparePreview&&<span className="preview-edit-hint">แตะและลากที่รูปเพื่อย้ายส่วนหัว · ใช้สองนิ้วเพื่อย่อ–ขยาย · หรือเลือก “ปรับหัว” ที่เมนู</span>}</>:a?<><img src={a} className="source-preview"/></>:<div className="preview-empty"><span className="add-photo">+ เพิ่มรูป</span><small>JPG · PNG · WEBP</small></div>}</label>
   {uniformCategory==='government'&&<div className="selected-job-uniform"><img src={selectedGovernmentTemplate} alt={'ชุด '+ministry}/><span>ชุดที่เลือก: {ministry}</span></div>}
   {uniformCategory==='government'&&ministry==='กระทรวงมหาดไทย'&&gender==='male'&&<div className="interior-template-picker process-interior-templates">{INTERIOR_UNIFORMS.map(t=><button type="button" key={t.id} className={selectedInteriorTemplate===t.img?'selected':''} onClick={()=>setSelectedInteriorTemplate(t.img)} disabled={busy||hairBusy||Boolean(b)} aria-label={t.name} aria-pressed={selectedInteriorTemplate===t.img}><img src={t.img} alt={t.name}/><span>{t.name}</span></button>)}</div>}
