...
function App(){
 ...
 return <main className="app-shell modern-shell"><header className="mobile-topbar"><button type="button" className="detail-back" onClick={()=>{setScreen('home');setHomeFilter(uniformCategory==='government'?'government':uniformCategory)}} aria-label="กลับหน้าก่อนหน้า">‹</button><div><div className="eyebrow">PHOTO READY</div><h1>{selectedStyle||'สร้างรูป'}</h1></div><div className="step-badge">ของฉัน</div></header><section className="modern-flow">
  <section className="style-detail-card"><div className="detail-title"><h2>เพิ่มรูป</h2><span>{selectedStyle||'แบบที่เลือก'}</span></div><label className="hero-preview preview-upload"><input type="file" accept="image/*" onChange={pick}/>{b?<img src={b}/>:a?<><img src={a} className="source-preview"/><span className="change-photo">เปลี่ยนรูป</span></>:<div className="preview-empty"><span className="add-photo">+ เพิ่มรูป</span><small>JPG · PNG · WEBP</small></div>}</label>
   ...
   <button className="primary-action create-now process-first" disabled={!f||busy} onClick={go}>{busy?'กำลังประมวลผล…':'ประมวลผลรูป'}</button>{msg&&<div className="err">{msg}</div>}{optionTool&&<div className="tool-choice-sheet">{optionTool==='head'?<><div className="tool-choice-tabs"><span className="active">ปรับหัว</span><span>ขนาดและตำแหน่ง</span></div><div className="compact-tool-panel process-head-adjust"><div className="compact-slider-list"><label><span>ขนาด</span><input type="range" min="20" max="200" step="1" value={Math.round(headAdjust.scale*100)} onChange={e=>applyAdjust({...headAdjust,scale:Number(e.target.value)/100})}/><b>{Math.round(headAdjust.scale*100)}%</b></label><label><span>ซ้าย–ขวา</span><input type="range" min="-50" max="50" step="0.5" value={headAdjust.x*100} onChange={e=>applyAdjust({...headAdjust,x:Number(e.target.value)/100})}/><b>{headAdjust.x>=0?'+':''}{(headAdjust.x*100).toFixed(1)}%</b></label><label><span>บน–ล่าง</span><input type="range" min="-50" max="50" step="0.5" value={headAdjust.y*100} onChange={e=>applyAdjust({...headAdjust,y:Number(e.target.value)/100})}/><b>{headAdjust.y>=0?'+':''}{(headAdjust.y*100).toFixed(1)}%</b></label></div></div></>:<><div className="tool-choice-tabs"><span className="active">{optionTool==='ribbon'?'แพรแถบ':optionTool==='male-hair'?'ทรงผมชาย':'ทรงผมหญิง'}</span><span>{optionTool==='ribbon'?'เลือกแบบ':'แตะรูปเพื่อเลือกทรง'}</span></div>{optionTool!=='ribbon'?<div className="hair-carousel process-thumbnail-strip">{HAIR_OPTIONS.map(h=><button type="button" key={h.id} className={'hair-card '+(hairId===h.id?'selected':'')} onClick={()=>setHairId(h.id)}><img src={h.src}/><span>{h.name.replace('ทรงผม ','')}</span></button>)}</div>:<div className="ribbon-choice-preview"><div className="ribbon-empty-thumb"><span>▤</span><strong>แพรแถบ</strong><small>รอเพิ่มไฟล์ PNG ตัวเลือก</small></div></div>}</>}<div className="tool-choice-footer"><button type="button" className="sheet-close" aria-label="ปิด" onClick={()=>setOptionTool(null)}>×</button><span>{optionTool==='head'?'ปรับหัว':optionTool==='ribbon'?'แพรแถบ':'ทรงผม'}</span><button type="button" className="sheet-confirm" aria-label="ยืนยัน" onClick={()=>setOptionTool(null)}>✓</button></div></div>}<div className="option-icon-bar process-option-bar simple-line-tools"><button type="button" className={optionTool==='head'?'active':''} onClick={()=>setOptionTool(optionTool==='head'?null:'head')}><span className="line-tool-icon" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M10 13c0-5 2-8 6-8s6 3 6 8v3c0 5-2 9-6 9s-6-4-6-9v-3Z"/><path d="M8 11c0-6 3-9 8-9s8 3 8 9M5 29c2-4 6-6 11-6s9 2 11 6"/><path d="M25 5v6M22 8h6"/></svg></span><strong>ปรับหัว</strong></button><button type="button" className={optionTool==='male-hair'?'active':''} onClick={()=>{setOptionTool('male-hair');setGender('male')}}><span className="line-tool-icon" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M9 13c0-5 3-8 7-8s7 3 7 8v3c0 5-3 9-7 9s-7-4-7-9v-3Z"/><path d="M7 14c1-7 4-11 9-11 6 0 9 5 9 12M5 29c1-4 5-6 11-6s10 2 11 6"/></svg></span><strong>ทรงผมชาย</strong></button><button type="button" className={optionTool==='female-hair'?'active':''} onClick={()=>{setOptionTool('female-hair');setGender('female')}}><span className="line-tool-icon" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M10 13c0-5 2-8 6-8s6 3 6 8v3c0 5-2 9-6 9s-6-4-6-9v-3Z"/><path d="M9 11C8 5 11 2 16 2s8 3 7 9c2 4 2 9 0 14M9 11C7 15 7 21 9 25M5 29c2-4 6-6 11-6s9 2 11 6"/></svg></span><strong>ทรงผมหญิง</strong></button><button type="button" className={optionTool==='ribbon'?'active':''} onClick={()=>setOptionTool('ribbon')}><span className="line-tool-icon" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M7 8h18v10H7zM10 18v9l6-4 6 4v-9M11 12h10M16 8v10"/></svg></span><strong>แพรแถบ</strong></button></div>
  </section>
  {b&&<section className="result-tools result-tools-v59"><div className="result-head"><div><h2>ปรับผลลัพธ์</h2><p>ปรับเฉพาะตำแหน่งหลังสร้าง</p></div><a className="save compact" href={b} download="photo-composed.png">ดาวน์โหลด</a></div><nav className="result-icon-tabs" aria-label="เครื่องมือปรับผลลัพธ์"><button type="button" className={resultTool==='head'?'active':''} onClick={()=>setResultTool('head')}><span className="result-line-icon"><svg viewBox="0 0 32 32"><path d="M10 13c0-5 2-8 6-8s6 3 6 8v3c0 5-2 9-6 9s-6-4-6-9v-3Z"/><path d="M8 11c0-6 3-9 8-9s8 3 8 9M5 29c2-4 6-6 11-6s9 2 11 6"/></svg></span><span>ส่วนหัว</span></button><button type="button" className={resultTool==='collar'?'active':''} onClick={()=>setResultTool('collar')}><span className="result-line-icon"><svg viewBox="0 0 32 32"><path d="M7 6l6 5 3 8 3-8 6-5 4 23H3L7 6Z"/><path d="M13 11l3 3 3-3M16 14v5"/></svg></span><span>ช่องคอ</span></button><button type="button" className={resultTool==='ribbon'?'active':''} onClick={()=>setResultTool('ribbon')}><span className="result-line-icon"><svg viewBox="0 0 32 32"><path d="M7 8h18v10H7zM10 18v9l6-4 6 4v-9M11 12h10M16 8v10"/></svg></span><span>แพรแถบ</span></button></nav><div className="post-tools compact-adjustments">{resultTool==='head'&&<div className="compact-tool-panel"><div className="compact-panel-head"><strong>จัดตำแหน่งส่วนหัว</strong><button type="button" className="tool-reset" onClick={()=>applyAdjust({scale:1,x:0,y:0})}>คืนค่า</button></div><div className="compact-slider-list"><label><span>ขนาด</span><input type="range" min="20" max="200" step="1" value={Math.round(headAdjust.scale*100)} onChange={e=>applyAdjust({...headAdjust,scale:Number(e.target.value)/100})}/><b>{Math.round(headAdjust.scale*100)}%</b></label><label><span>ซ้าย–ขวา</span><input type="range" min="-50" max="50" step="0.5" value={headAdjust.x*100} onChange={e=>applyAdjust({...headAdjust,x:Number(e.target.value)/100})}/><b>{headAdjust.x>=0?'+':''}{(headAdjust.x*100).toFixed(1)}%</b></label><label><span>บน–ล่าง</span><input type="range" min="-50" max="50" step="0.5" value={headAdjust.y*100} onChange={e=>applyAdjust({...headAdjust,y:Number(e.target.value)/100})}/><b>{headAdjust.y>=0?'+':''}{(headAdjust.y*100).toFixed(1)}%</b></label></div></div>}{resultTool==='collar'&&<div className="compact-tool-panel"><div className="compact-panel-head"><strong>ช่องคอชุด</strong><button type="button" className="tool-reset" onClick={()=>applyCollarWarp(0)}>คืนค่า</button></div><div className="collar-compact-actions"><button type="button" className="collar-auto compact-auto" onClick={autoFitCollar}>พอดีอัตโนมัติ</button></div><div className="compact-slider-list"><label><span>หุบ–ขยาย</span><input type="range" min="-100" max="100" step="1" value={Math.round(collarWarp*100)} onChange={e=>applyCollarWarp(Number(e.target.value)/100)}/><b>{Math.round(collarWarp*100)}%</b></label></div></div>}{resultTool==='ribbon'&&<div className="compact-tool-panel ribbon-compact"><strong>แพรแถบ</strong><span>โครง Layer พร้อมเพิ่ม PNG ภายหลัง</span><small>เร็ว ๆ นี้</small></div>}</div></section>}
 </section></main>
}
