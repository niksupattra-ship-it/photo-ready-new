...
function App(){
 ...
 return <main className="app-shell modern-shell"><header className="mobile-topbar"><button type="button" className="detail-back" onClick={()=>setScreen('category')}>‹</button><div><div className="eyebrow">PHOTO READY</div><h1>{selectedStyle||'สร้างรูป'}</h1></div><div className="step-badge">ของฉัน</div></header><section className="modern-flow">
  <section className="style-detail-card"><div className="detail-title"><h2>เพิ่มรูป</h2><span>{selectedStyle||'แบบที่เลือก'}</span></div><label className="hero-preview preview-upload"><input type="file" accept="image/*" onChange={pick}/>{b?<img src={b}/>:a?<><img src={a} className="source-preview"/><span className="change-photo">เปลี่ยนรูป</span></>:<div className="preview-empty"><span className="add-photo">+ เพิ่มรูป</span><small>JPG · PNG · WEBP</small></div>}</label>
   ...
   <button className="primary-action create-now process-first" disabled={!f||busy} onClick={go}>{busy?'กำลังประมวลผล…':'ประมวลผลรูป'}</button>{msg&&<div className="err">{msg}</div>}<div className="option-icon-bar"><button type="button" className="active"><span>✂</span><strong>ทรงผม</strong></button><button type="button"><span>{gender==='male'?'♂':'♀'}</span><strong>{gender==='male'?'ชาย':'หญิง'}</strong></button>{uniformCategory==='government'&&<button type="button"><span>▤</span><strong>แพรแถบ</strong></button>}</div><div className="hair-picker-head"><div><h3>ทรงผม</h3><p>เลือกจาก 29 แบบ</p></div><span>{hairId.replace('hair-','')}</span></div><div className="hair-carousel">{HAIR_OPTIONS.map(h=><button type="button" key={h.id} className={'hair-card '+(hairId===h.id?'selected':'')} onClick={()=>setHairId(h.id)}><img src={h.src}/><span>{h.name.replace('ทรงผม ','')}</span></button>)}</div>
  </section>
