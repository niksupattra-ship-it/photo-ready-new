...
function App(){
 ...
 const setPreviewViewZoom=next=>{const zoom=Math.max(1,Math.min(4,next));setPreviewZoom(zoom);setPreviewPan(current=>zoom===1?{x:0,y:0}:clampPreviewPan(current,zoom))};
 ...
 const previewPointerMove=e=>{
  if(viewMode){
   ...
   if(v.pointers.size>=2&&v.pinch){const pts=[...v.pointers.values()].slice(0,2),distance=Math.hypot(pts[0].x-pts[1].x,pts[0].y-pts[1].y),midX=(pts[0].x+pts[1].x)/2,midY=(pts[0].y+pts[1].y)/2,zoom=Math.max(1,Math.min(4,v.startZoom*distance/Math.max(1,v.distance)));setPreviewZoom(zoom);setPreviewPan(clampPreviewPan({x:v.startPan.x+(midX-v.midX),y:v.startPan.y+(midY-v.midY)},zoom))}
...
<button type="button" className={comparePreview?'active':''} onPointerDown={e=>e.stopPropagation()} onClick={e=>{e.preventDefault();e.stopPropagation();setComparePreview(v=>!v)}} aria-label="เปรียบเทียบ"><span>◐</span><small>เปรียบเทียบ</small></button></div>{!comparePreview&&<span className="preview-edit-hint">{viewMode?'กำลังซูมทั้งกรอบ · ลากเพื่อเลื่อน · เลือกเครื่องมือเพื่อปรับต่อในมุมซูม':optionTool==='ribbon'?'แตะและลากแพรแถบเพื่อย้ายตำแหน่ง · ปรับละเอียดได้ในเมนูแพรแถบ':'แตะและลากที่รูปเพื่อย้ายส่วนหัว · ใช้สองนิ้วเพื่อย่อ–ขยาย · หรือเลือก “ปรับหัว” ที่เมนู'}</span>}</>:a?<><img src={a} className="source-preview"/></>:<div className="preview-empty"><span className="add-photo">+ เพิ่มรูป</span><small>JPG · PNG · WEBP</small></div>}{processProgress.active&&<div className="image-progress-overlay" role="status" aria-live="polite" onClick={e=>{e.preventDefault();e.stopPropagation()}}><div className="image-progress-card"><div className="image-progress-copy"><span>{processProgress.label}</span><strong>{Math.round(processProgress.value)}%</strong></div><div className="image-progress-track"><i style={{width:`${processProgress.value}%`}}/></div></div></div>}{(a||b)&&<div className="preview-view-controls" onPointerDown={e=>{e.preventDefault();e.stopPropagation()}} onClick={e=>{e.preventDefault();e.stopPropagation()}}><button type="button" className={viewMode?'active':''} onClick={()=>setViewMode(v=>!v)} aria-label={viewMode?'กลับไปปรับภาพโดยคงระดับซูม':'เปิดโหมดซูมทั้งกรอบ'}><span>⌕</span><small>{viewMode?'ปรับต่อ':'ซูม'}</small></button>{viewMode&&<><button type="button" onClick={()=>setPreviewViewZoom(previewZoom-.25)} aria-label="ซูมออก">−</button><output>{Math.round(previewZoom*100)}%</output><button type="button" onClick={()=>setPreviewViewZoom(previewZoom+.25)} aria-label="ซูมเข้า">+</button><button type="button" onClick={resetPreviewView} aria-label="คืนมุมมอง 100%">100%</button></>}</div>}</label>
