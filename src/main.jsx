...
function App(){
 ...
 return <main className="app-shell modern-shell"><header className="mobile-topbar"><button type="button" className="detail-back" onClick={()=>{setScreen('home');setHomeFilter(uniformCategory==='government'?'government':uniformCategory)}} aria-label="กลับหน้าก่อนหน้า">‹</button><div><div className="eyebrow">PHOTO READY</div><h1>{selectedStyle||'สร้างรูป'}</h1></div><div className="step-badge">ของฉัน</div></header><section className="modern-flow">
  ...
  {b&&<section className="result-tools"><div className="result-head"><div><h2>ปรับผลลัพธ์</h2><p>ปรับเฉพาะตำแหน่งหลังสร้าง</p></div><a className="save compact" href={b} download="photo-composed.png">ดาวน์โหลด</a></div><div className="post-tools"><div className="head-tools"><div className="tool-panel-header"><div><div className="head-tools-title">จัดตำแหน่งส่วนหัว</div><div className="tool-subtitle">ปรับขนาดและตำแหน่ง</div></div><button type="button" className="tool-reset" onClick={()=>applyAdjust({scale:1,x:0,y:0})}>คืนค่า</button></div><div className="slider-stack"><label className="slider-row"><div className="slider-meta"><span>ขนาด</span><strong>{Math.round(headAdjust.scale*100)}%</strong></div><input type="range" min="20" max="200" step="1" value={Math.round(headAdjust.scale*100)} onChange={e=>applyAdjust({...headAdjust,scale:Number(e.target.value)/100})}/></label><label className="slider-row"><div className="slider-meta"><span>ซ้าย — ขวา</span><strong>{headAdjust.x>=0?'+':''}{(headAdjust.x*100).toFixed(1)}%</strong></div><input type="range" min="-50" max="50" step="0.5" value={headAdjust.x*100} onChange={e=>applyAdjust({...headAdjust,x:Number(e.target.value)/100})}/></label><label className="slider-row"><div className="slider-meta"><span>บน — ล่าง</span><strong>{headAdjust.y>=0?'+':''}{(headAdjust.y*100).toFixed(1)}%</strong></div><input type="range" min="-50" max="50" step="0.5" value={headAdjust.y*100} onChange={e=>applyAdjust({...headAdjust,y:Number(e.target.value)/100})}/></label></div></div><div className="collar-warp-panel"><div className="tool-panel-header"><div><div className="collar-warp-title">ช่องคอชุด</div><div className="tool-subtitle">บิดเฉพาะ Template ชุด</div></div><button type="button" className="tool-reset" onClick={()=>applyCollarWarp(0)}>คืนค่า</button></div><button type="button" className="collar-auto" onClick={autoFitCollar}>พอดีอัตโนมัติ</button><label className="slider-row collar-slider"><div className="slider-meta"><span>หุบ — ขยาย</span><strong>{Math.round(collarWarp*100)}%</strong></div><input type="range" min="-100" max="100" step="1" value={Math.round(collarWarp*100)} onChange={e=>applyCollarWarp(Number(e.target.value)/100)}/></label></div><div className="ribbon-panel"><div><strong>แพรแถบ</strong><small>โครง Layer พร้อมเพิ่ม PNG ภายหลัง</small></div><span>เร็ว ๆ นี้</span></div></div></section>}
 </section></main>
}
